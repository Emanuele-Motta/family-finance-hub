...
CREATE TABLE IF NOT EXISTS public.audit_logs (
  ...
  entity_type TEXT NOT NULL, -- 'transaction', 'budget', 'category', etc.
  entity_id UUID,
  entity_name TEXT,
  old_values JSONB,
  new_values JSONB,
...
CREATE TABLE IF NOT EXISTS public.record_versions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  family_group_id UUID NOT NULL REFERENCES public.family_groups(id) ON DELETE CASCADE,
  record_type TEXT NOT NULL, -- 'transaction', 'budget', 'category', etc.
...
-- Import batches (CSV/bank feeds)
CREATE TABLE IF NOT EXISTS public.import_batches (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  family_group_id UUID NOT NULL REFERENCES public.family_groups(id) ON DELETE CASCADE,
  account_id UUID NOT NULL REFERENCES public.accounts(id) ON DELETE CASCADE,
  imported_by UUID NOT NULL REFERENCES auth.users(id) ON DELETE SET NULL,
  import_source TEXT NOT NULL, -- 'csv', 'bank_feed', 'manual'
  total_rows INT NOT NULL DEFAULT 0,
  processed_rows INT NOT NULL DEFAULT 0,
  status TEXT NOT NULL DEFAULT 'pending', -- pending, processing, reviewed, imported, rejected
  metadata JSONB, -- file info, mapping, etc.
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.import_pending_transactions (
  ...
  tags TEXT[], -- proposed tags
  notes TEXT,
  status TEXT NOT NULL DEFAULT 'pending', -- pending, matched, duplicate_warning, manual_edit, confirmed
  matched_transaction_id UUID REFERENCES public.transactions(id) ON DELETE SET NULL,
  duplicate_score NUMERIC(3,2), -- 0.0-1.0 confidence
...
CREATE TABLE IF NOT EXISTS public.reconciliations (
  ...
  matched_transaction_id UUID REFERENCES public.transactions(id) ON DELETE SET NULL,
  match_confidence NUMERIC(3,2) NOT NULL DEFAULT 0, -- 0.0-1.0
  match_method TEXT NOT NULL, -- 'auto', 'manual_confirm', 'auto_confirmed'
  match_score_details JSONB, -- debugging info
...
-- ============================================================================
-- AUTOMATIC RULES (GMAIL-STYLE FILTERS)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.transaction_rules (
  ...
  condition_type TEXT NOT NULL, -- 'all_match' or 'any_match'
  conditions JSONB NOT NULL, -- array of {field, operator, value}
  -- fields: description, amount, date, type, account_id, tags, etc.
  -- operators: contains, not_contains, equals, gt, lt, gte, lte, in, not_in, regex
...
CREATE TABLE IF NOT EXISTS public.recurring_templates (
  ...
  interval INT NOT NULL DEFAULT 1, -- every N cycles
  day_of_month INT, -- for monthly: 1-31, NULL=use first occurrence
  day_of_week TEXT, -- for weekly: 'monday', 'tuesday', etc.
  months TEXT[], -- for custom: ['jan', 'feb', ...]
  ...
  -- Notifications
  notify_days_before INT, -- reminder N days before
  notify_method TEXT, -- 'push', 'telegram', 'email', 'all'
...
CREATE TABLE IF NOT EXISTS public.cashflow_forecasts (
  ...
  calculation_method TEXT NOT NULL, -- 'recurring', 'avg_historical', 'combined'
  metadata JSONB, -- debugging/details
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.transaction_comments (
  ...
  -- Settlement tracking
  is_settlement_comment BOOLEAN NOT NULL DEFAULT false, -- "User A paid User B back"
...
CREATE TABLE IF NOT EXISTS public.transaction_approvals (
  ...
  approval_threshold NUMERIC(12,2) NOT NULL, -- amount that triggered approval
...
CREATE TABLE IF NOT EXISTS public.notifications (
  ...
  notification_type TEXT NOT NULL, -- 'budget_alert', 'recurring_reminder', 'anomaly', 'approval_needed', 'settlement_reminder', 'milestone'
  title TEXT NOT NULL,
  message TEXT NOT NULL,
  related_entity_type TEXT, -- 'transaction', 'budget', 'goal', 'recurring', etc.
  related_entity_id UUID,
  
  -- Delivery tracking
  status TEXT NOT NULL DEFAULT 'created', -- created, queued, sent, failed
  delivery_channels TEXT[] NOT NULL DEFAULT '{}', -- 'push', 'telegram', 'email'
  sent_at TIMESTAMPTZ,
  read_at TIMESTAMPTZ,
...
CREATE TABLE IF NOT EXISTS public.anomalies (
  ...
  anomaly_type TEXT NOT NULL, -- 'unusual_amount', 'unusual_category', 'duplicate_like', 'fraud_score'
  severity TEXT NOT NULL DEFAULT 'info', -- 'info', 'warning', 'critical'
  description TEXT NOT NULL,
  confidence NUMERIC(3,2) NOT NULL DEFAULT 0, -- 0.0-1.0
  analysis JSONB, -- details: historical avg, deviation %, similar transactions, etc.
...
-- Function to create record version
